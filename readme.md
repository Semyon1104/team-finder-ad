...
## 5. Инструкция для ревьюера

Проект реализован для `TASK_VERSION=3` (необходимые навыки проектов и фильтрация проектов по навыкам).

### Быстрый запуск

1. Создайте `.env` (можно на базе примера):
   ```bash
   cp .env_example .env
   ```
   Убедитесь, что в `.env` стоит `TASK_VERSION=3` и корректные параметры PostgreSQL.
...
4. Примените миграции:
   ...
   ```
5. Создайте тестовые данные для проверки - пользователи и проекты:
   ```bash
   python manage.py seed_demo_data
   ```
6. Создайте суперпользователя (опционально, для проверки админки):
   ```bash
...
- Регистрация (`/users/register/`) и вход (`/users/login/`) по email/паролю.
- Главная (`/projects/list/`): карточки проектов, сортировка новые сверху, пагинация по 12.
- Пользователи (`/users/list/`): карточки и пагинация по 12, сортировка по id.
- Проект (`/projects/<id>/`): участие/отказ, редактирование и завершение для владельца.
- Навыки проекта: автодополнение (`/projects/skills/?q=`), добавление (`/projects/<id>/skills/add/`) и удаление (`/projects/<id>/skills/<skill_id>/remove/`) без перезагрузки.
- Главная (`/projects/list/`): фильтр по необходимым навыкам через `?skill=<название>`.
- Профиль (`/users/<id>/`), редактирование профиля и смена пароля.

### Тестовые данные и автотесты

- Быстрое создание нескольких пользователей и проектов:
  ```bash
  python manage.py seed_demo_data
  ```
  Команда создаёт 3 демо-пользователя и минимум по одному проекту от каждого.
- Пароль для новых демо-пользователей: `DemoPass123!`
- Запуск автотестов:
  ```bash
...
В проект добавлен workflow `.github/workflows/ci.yml`, который на `push` и `pull_request`:
- поднимает PostgreSQL сервис;
- устанавливает зависимости;
- выполняет `python manage.py migrate --noinput`;
- запускает `python manage.py check` и `python manage.py test`.
